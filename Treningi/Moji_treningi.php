...
while($row = mysqli_fetch_assoc($get)){
  ...
  if($row['prisotnost'] == 0){
    if($row['priponka'] != '')$table .= "<tr><td>".$row['naslov']."</td>"."<td>".date("d.m.Y",strtotime($row['datum'])). " ".substr($row['zacetek'],strpos($row['zacetek'],"T")+1)."</td>"."<td>".$imeEkipe[0]."</td>"."<td>".$row['uvod']."</td>"."<td>".$row['glavni']."</td>"."<td>".$row['zakljucek']."</td><td>".$row['porocilo']. " <a href='./" . $row['priponka'] .   "' />Priponka</td><td><button type=button class='btn btn-primary priso'  data-toggle=modal  value=".$idTreninga." data-target=#vnosPrisotnosti>Vnesi prisotne</td></tr>";
    else $table .= "<tr><td>".$row['naslov']."</td>"."<td>".date("d.m.Y",strtotime($row['datum'])). " ".substr($row['zacetek'],strpos($row['zacetek'],"T")+1)."</td>"."<td>".$imeEkipe[0]."</td>"."<td>".$row['uvod']."</td>"."<td>".$row['glavni']."</td>"."<td>".$row['zakljucek']."</td><td>".$row['porocilo']."</td><td><button type=button class='btn btn-primary priso'  data-toggle=modal  value=".$idTreninga." data-target=#vnosPrisotnosti>Vnesi prisotne</td></tr>";
  }
  else{
    if($row['priponka'] != '') $table .= "<tr><td><a href=Trening.php?id=".$idTreninga.">".$row['naslov']."</a></td>"."<td>".date("d.m.Y",strtotime($row['datum'])). " ".substr($row['zacetek'],strpos($row['zacetek'],"T")+1)."</td>"."<td>".$imeEkipe[0]."</td>"."<td>".$row['uvod']."</td>"."<td>".$row['glavni']."</td>"."<td>".$row['zakljucek']."</td><td>".$row['porocilo']. " " . $row['priponka'] . " <a href='./" . $row['priponka'] .   "' />Priponka</td><td>".$prisotni." / ". $stvseh ." (".$procent ."%)" ."</td></tr>";
    else $table .= "<tr><td><a href=Trening.php?id=".$idTreninga.">".$row['naslov']."</a></td>"."<td>".date("d.m.Y",strtotime($row['datum'])). " ".substr($row['zacetek'],strpos($row['zacetek'],"T")+1)."</td>"."<td>".$imeEkipe[0]."</td>"."<td>".$row['uvod']."</td>"."<td>".$row['glavni']."</td>"."<td>".$row['zakljucek']."</td><td>".$row['porocilo']. " " . $row['priponka'] .    "</td><td>".$prisotni." / ". $stvseh ." (".$procent ."%)" ."</td></tr>";
  }
